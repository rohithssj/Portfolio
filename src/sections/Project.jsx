import React, { useState } from 'react'
import ProjectDetails from './ProjectDetails'

const Project = ({ title, description, subDescription, href, image, tags = [] }) => {
    // Fixed: Standardized React state naming convention (state, setter)
    const [isOpen, setIsOpen] = useState(false)

    return (
        <>
            <div className='flex-wrap items-center py-10 justify-between space-y-14 sm:flex sm:space-y-0'>
                <div>
                    <p className='text-2xl font-semibold'>{title}</p>
                    <div className="flex flex-wrap gap-3 mt-2 text-sand">
                        {tags.map((tag) => (
                            <span key={tag.id || tag.name || tag} className="text-sm">
                                {tag.name || tag}
                            </span>
                        ))}
                    </div>
                </div>
                
                <button 
                    onClick={() => setIsOpen(true)} 
                    className='flex items-center gap-1 cursor-pointer hover-animation transition-transform hover:translate-x-1'
                >
                    Read More
                    <img src="/assets/arrow-right.svg" className='w-5' alt="Arrow right" />
                </button>
            </div>
            
            <div className='bg-gradient-to-r from-transparent via-neutral-700 to-transparent h-[1px] w-full'/>

            {isOpen && ( 
                <ProjectDetails 
                    title={title}
                    description={description}
                    subDescription={subDescription}
                    image={image}
                    href={href}
                    tags={tags}
                    close={() => setIsOpen(false)} 
                />
            )}
        </>
    )
}

export default Project
